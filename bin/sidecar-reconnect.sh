...
# @raycast.title Reconnect iPad Sidecar
# @raycast.mode fullOutput
# @raycast.packageName Sidecar
# @raycast.description Disconnect and reconnect an iPad when Sidecar is stuck
...
if ! "$launcher" disconnect "$device"; then
  echo "Disconnect failed. Trying to connect anyway."
fi
sleep 2

echo "Reconnecting to $device…"
if "$launcher" connect "$device"; then
  echo "Sidecar reconnected."
else
